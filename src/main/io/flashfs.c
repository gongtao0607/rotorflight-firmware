...
uint32_t flashfsGetUsage(void)
{
    ...
    return (tailAddress + bufferSizes[0] + bufferSizes[1] + flashfsSize -
            headAddress) % flashfsSize;
}
...
static bool flashfsIsPageErased(uint32_t address){
    ...
    if (flashReadBytes(address, testBuffer.bytes, FREE_BLOCK_TEST_SIZE_BYTES) < FREE_BLOCK_TEST_SIZE_BYTES) {
    ...
    for (uint8_t i = 0; i < FREE_BLOCK_TEST_SIZE_INTS; i++) {
...
bool flashfsIsEOF(void)
{
    ...
    int32_t persistedBytes = tailAddress - headAddress;
    if (persistedBytes < 0)
        persistedBytes += flashfsSize;

    return (uint32_t) persistedBytes >=
